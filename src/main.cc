...
// Taken from ray tracer project
bool intersect_box(vec3 p, vec3 d, out vec3 intersect_p, out vec3 intersect_n) {

    float RAY_EPSILON = 0.00000001;
    float HUGE_DOUBLE = 1e100;
    ...
    intersect_p = p + (bestT * d);

    int i1 = (bestIndex + 1) % 3;
    int i2 = (bestIndex + 2) % 3;

    if (bestIndex < 3) {
        // i.setN(glm::dvec3(-double(bestIndex == 0), -double(bestIndex == 1),
        //                  -double(bestIndex == 2)));
        // i.setUVCoordinates(glm::dvec2(0.5 - intersect_point[min(i1, i2)],
        intersect_n = vec3(-float(bestIndex == 0), -float(bestIndex == 1),
                          -float(bestIndex == 2));
    } else {
        // i.setN(glm::dvec3(double(bestIndex == 3), double(bestIndex == 4),
        //                  double(bestIndex == 5)));
        // i.setUVCoordinates(glm::dvec2(0.5 + intersect_point[min(i1, i2)],
        //                              0.5 + intersect_point[max(i1, i2)]));
        intersect_n = vec3(float(bestIndex == 3), float(bestIndex == 4),
                          float(bestIndex == 5));
    }
    return true;
}
